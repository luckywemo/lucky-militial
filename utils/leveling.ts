...
export function calculateXP(stats: LevelStats): number {
    const k = isNaN(Number(stats.kills)) ? 0 : Number(stats.kills);
    const w = isNaN(Number(stats.wins)) ? 0 : Number(stats.wins);
    const g = isNaN(Number(stats.gamesPlayed)) ? 0 : Number(stats.gamesPlayed);
    
    return (k * XP_PER_KILL) + (w * XP_PER_WIN) + (g * XP_PER_GAME);
}
